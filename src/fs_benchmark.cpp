...
struct Options {
	std::filesystem::path root = "D:/dev/boost_1_90_0";
...
Options parse_options(int argc, char** argv) {
	...
	for (int index = 1; index < argc; ++index) {
		...
		if (arg == "--help" || arg == "-h") {
			...
			std::cout << "  root-path     Directory to traverse, default is D:/dev/boost_1_90_0\n";
			exit(0);
		}
		options.root = arg;
	}
	...
	try {
		...
		while (auto meta = meta_queue.pop()) {
			...
			if (options.read_files) {
				soratransport::FileReader reader(pool, meta->full_path);
				bytes_read += read_file_and_discard(reader, *meta);
			}
		}
