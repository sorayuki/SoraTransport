...
void ZstdCompressor::compress(ConcurrentDataChunkChannel& in_tar, IByteSink& sink) {
	auto result = executors_.post_compression([this, &in_tar, &sink] {
		compress_sync(in_tar, sink);
	});
	result.get();
...
void ZstdCompressor::compress_sync(ConcurrentDataChunkChannel& in_tar, IByteSink& sink) {
	auto* context = ZSTD_createCCtx();
	if (context == nullptr) {
		throw std::runtime_error("failed to create zstd compression context");
	}
	...
	try {
		...
		while (auto chunk = receive_chunk_blocking(in_tar)) {
			ZSTD_inBuffer input{chunk->data.get(), chunk->length, 0};
			while (input.pos < input.size) {
				ZSTD_outBuffer output{output_buffer.get(), output_capacity, 0};
				const auto result = ZSTD_compressStream2(context, &output, &input, ZSTD_e_continue);
...
void ZstdDecompressor::decompress(IByteSource& source, ConcurrentDataChunkChannel& out_tar) {
	auto* context = ZSTD_createDCtx();
	if (context == nullptr) {
		throw std::runtime_error("failed to create zstd decompression context");
	}
	...
	try {
		while (!eof || input.pos < input.size) {
			...
			if (output.pos > 0) {
				send_chunk_blocking(out_tar, DataChunk{std::move(output_buffer), output.pos, offset, false});
				offset += output.pos;
			}
			if (eof && input.pos == input.size && result == 0) {
				break;
...
void RawTarWriter::write(ConcurrentDataChunkChannel& in_tar, IByteSink& sink) {
	while (auto chunk = receive_chunk_blocking(in_tar)) {
		sink.write({chunk->data.get(), chunk->length});
	}
	sink.close();
...
void RawTarReader::read(IByteSource& source, ConcurrentDataChunkChannel& out_tar) {
	std::uint64_t offset = 0;
	try {
		for (;;) {
			auto buffer = pool_.acquire(kPipelineChunkSize);
			...
			send_chunk_blocking(out_tar, DataChunk{std::move(buffer), bytes_read, offset, false});
			offset += bytes_read;
		}
		out_tar.close();
	} catch (...) {
